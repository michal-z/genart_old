...
inline VECTOR4 &operator+=(VECTOR4 &a, const VECTOR4 &b) { return a = a + b; }
inline VECTOR2 &operator-=(VECTOR2 &a, const VECTOR2 &b) { return a = a - b; }
inline VECTOR3 &operator-=(VECTOR3 &a, const VECTOR3 &b) { return a = a - b; }
inline VECTOR4 &operator-=(VECTOR4 &a, const VECTOR4 &b) { return a = a - b; }
...
Get_And_Reset_Command_List(DX12_CONTEXT &dx)
{
    dx.cmdalloc[dx.frame_index]->Reset();
    dx.cmdlist->Reset(dx.cmdalloc[dx.frame_index], nullptr);
    return dx.cmdlist;
}

inline void
Allocate_Descriptors(DX12_CONTEXT &dx,
                     D3D12_DESCRIPTOR_HEAP_TYPE type,
                     u32 count,
                     D3D12_CPU_DESCRIPTOR_HANDLE& handle)
{
    u32 descriptor_size;
    DESCRIPTOR_HEAP &heap = Get_Descriptor_Heap(dx, type, D3D12_DESCRIPTOR_HEAP_FLAG_NONE, descriptor_size);
...
inline void
Allocate_GPU_Descriptors(DX12_CONTEXT &dx,
                         u32 count,
                         D3D12_CPU_DESCRIPTOR_HANDLE &cpu_handle,
                         D3D12_GPU_DESCRIPTOR_HANDLE &gpu_handle)
{
    u32 descriptor_size;
    DESCRIPTOR_HEAP &heap = Get_Descriptor_Heap(dx, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,
...
Copy_Descriptors_To_GPU(DX12_CONTEXT &dx,
                        u32 count,
                        D3D12_CPU_DESCRIPTOR_HANDLE src_base)
{
    D3D12_CPU_DESCRIPTOR_HANDLE cpu_dst_base;
    D3D12_GPU_DESCRIPTOR_HANDLE gpu_dst_base;
    Allocate_GPU_Descriptors(dx, count, cpu_dst_base, gpu_dst_base);
    dx.device->CopyDescriptorsSimple(count, cpu_dst_base, src_base, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    return gpu_dst_base;
}

inline void
Get_Back_Buffer(DX12_CONTEXT &dx,
                ID3D12Resource *&buffer,
                D3D12_CPU_DESCRIPTOR_HANDLE &handle)
{
    buffer = dx.swapbuffers[dx.back_buffer_index];
    handle = dx.rt_heap.cpu_start;
    handle.ptr += dx.back_buffer_index * dx.descriptor_size_rtv;
}

inline void
Get_Depth_Stencil_Buffer(DX12_CONTEXT &dx,
                         ID3D12Resource *&buffer,
                         D3D12_CPU_DESCRIPTOR_HANDLE &handle)
{
    buffer = dx.ds_buffer;
    handle = dx.ds_heap.cpu_start;
}

inline void
Bind_GPU_Descriptor_Heap(const DX12_CONTEXT &dx)
{
    dx.cmdlist->SetDescriptorHeaps(1, &dx.gpu_descriptor_heaps[dx.frame_index].heap);
}
#endif // USE_DX12
